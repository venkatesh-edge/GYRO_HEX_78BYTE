...
def capitalize_alphabets_in_data(data):
    ...
    return bytes(
        [ord(chr(byte).upper()) if 65 <= byte <= 90 or 97 <= byte <= 122 else byte for byte in data]
    )
...
def generate_mock_data_stream():
    """
    Generates a mock data stream containing valid packets, random noise, and incomplete packets.
    """
    stream = bytearray()

    # Add random noise (random bytes)
    noise = [random.randint(0, 255) for _ in range(random.randint(10, 30))]
    stream.extend(noise)

    # Add a valid packet
    stream.extend(generate_valid_packet())

    # Add an incomplete packet
    incomplete_packet = generate_valid_packet()[:random.randint(1, 77)]
    stream.extend(incomplete_packet)

    # Add more random noise
    noise = [random.randint(0, 255) for _ in range(random.randint(5, 15))]
    stream.extend(noise)
...
def main():
    ...
    try:
        ...
        with serial.Serial(
            ...
            while True:
                mock_data = generate_mock_data_stream()
